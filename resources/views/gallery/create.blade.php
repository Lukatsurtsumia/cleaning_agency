<x-site-layout>
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <h1 class="text-3xl font-bold">Add Project</h1>
        ...
        <form method="POST" action="{{ route('gallery.store') }}" enctype="multipart/form-data" class="mt-8 space-y-6">
            @csrf
            ...
            <div>
                ...
                <select name="category" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                    @foreach ($categories as $category)
                        <option value="{{ $category }}" @selected(old('category') === $category)>{{ collect(__('site.services.items'))->firstWhere('key', $category)['name'] ?? Str::headline($category) }}</option>
                    @endforeach
                </select>
            </div>
